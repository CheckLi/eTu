 ...
 */
class InputPriceTwoDialog : Dialog {
    ...
    constructor(context: Context, title: String) : super(context, R.style.LoadingDialog) {
        ...
        tv_input_price2.visibility=View.VISIBLE
    ...
    fun setRightBtnResultText(content: String, tips: String, onClick: (content1: String,content2: String) -> Unit) {
        btn_yes.text = content
        btn_yes.setOnClickListener {
            if (!tv_input_price.text.isNullOrBlank()) {
                onClick(tv_input_price.text.toString(),tv_input_price2.toString())
            } else {
                context.showToast(tips)
            }
        }
