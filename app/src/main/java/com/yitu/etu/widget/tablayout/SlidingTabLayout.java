...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
     ...
     * 关联ViewPager
     */
    public void setViewPager(ViewPager vp, int[] mDrawables) {
        ...
        if (mDrawables.length != vp.getAdapter().getCount()) {
            throw new IllegalStateException("Titles length must be the same as the page count !");
        }
        this.mViewPager = vp;
        this.mDrawables = new ArrayList<>();
        for (int mDrawable : mDrawables) {
            this.mDrawables.add(mDrawable);
        }
    ...
    private void addTab(final int position, String title, View tabView) {
        ...
        int drawId = -1;
        if (mDrawables != null) {
            drawId = mDrawables.get(position);
            boolean isSelect = getCurrentTab() == position;
            if (drawId != -1 && isSelect) {
                Drawable drawable = getResources().getDrawable(drawId);
                drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());
                drawable.setColorFilter(mTextSelectColor, PorterDuff.Mode.SRC_IN);
                tv_tab_title.setCompoundDrawables(null, drawable, null, null);
            } else if (drawId != -1 && !isSelect) {
                Drawable drawable = getResources().getDrawable(drawId);
                drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());
                drawable.setColorFilter(mTextUnselectColor, PorterDuff.Mode.SRC_IN);
                tv_tab_title.setCompoundDrawables(null, drawable, null, null);
            } else {
                tv_tab_title.setCompoundDrawables(null, null, null, null);
            }
        }
        ...
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (mDrawables != null) {
                ...
                if (drawId != -1 && isSelect) {
                    ...
                    tab_title.setCompoundDrawables(null, null, null, null);
                }
            }
            if (tab_title != null) {
